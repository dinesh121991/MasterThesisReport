Wait<-function(F1,F2)
{
	...
	postscript("cdf-exec-SLURM-1024-16384_2.eps",width=8,height=5,onefile = TRUE, horizontal = FALSE,paper="special")
#	png(file="Cumulated Distribution function on Wait time(OAR-SLURM).png",height=850,width=1000)
	
	plot(ecdf(sort(Wait1)),pch=22, xlim=c(0,max(Wait1,Wait2)),xlab="Execution time [s]",ylab="Jobs [%]", main= "CDF on Execution time for Light-ESP benchmark (230 jobs)\nupon a 16 cpu/node cluster with variation on the number of nodes \n (emulation with 400 physical nodes)")
#plot(ecdf(sort(Wait1)),do.points =F, verticals= T,xlim=c(0,max(Wait1,Wait2)),xlab="Wait time [s]",ylab="Jobs [%]", main= "Cumulated Distribution function on Wait time")	
	par(new=T)
	plot(ecdf(sort(Wait2)),pch=20, xlim=c(0,max(Wait1,Wait2)),xlab="",ylab="", main= "")	
...
#	mtext(paste("files :",F1, "-", F2)) 
	legend("bottomright",c("cons_res","cons_res_power"),pch=c(22,20),lty=c(1,0),inset=0.02)
	dev.off()
...
#	dev.off()
	...
	png(file="Exec_time_cr_crlp.png",height=850,width=1000)
  ...
  #	colnames(m)<-h1$breaks[2:length(h1$breaks)]
   	...
   	barplot(m,													# matrix
  		...
  		col=c("pink","green") ,					# colors of the histograms
	...
	mtext(paste("Files :",F1,"-",F2))	
  	...
  	legend("top",														# where to put the legend
  		...
  		pch=c(22,20),															# we want the symbol 22 for the legend
...
Wait("cons_res_no_overhead.swf","cons_res_layout_power_2.swf")
